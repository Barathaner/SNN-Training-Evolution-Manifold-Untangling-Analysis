...
def create_umap_embedding(dataloader, n_neighbors=15, min_dist=0.2, random_state=42, max_samples_per_class=None):
    """
    ...
    # Jede Trajektorie (Sample) wird ein Datenpunkt
    # Flatten jede Trajektorie zu einem Vektor: (T, features) -> (T * features,)
    X_list = []
    labels_list = []
    
    for sample_data, sample_label in zip(all_data, all_sample_labels):
        # Flatten Trajektorie zu einem Vektor
        trajectory_vector = sample_data.flatten()  # (T * features,)
        X_list.append(trajectory_vector)
        labels_list.append(sample_label)  # Ein Label pro Trajektorie
    
    X = np.array(X_list)  # (N_samples, T * features)
    labels = np.array(labels_list)  # (N_samples,)
    
    print(f"      Gesamt: {len(X)} Trajektorien (jede mit {all_data[0].shape[0]} Zeitbins)")
